...
class ASTModel(nn.Module):
    ...
    def finetuningemb(self, x):
        ...
        # x.shape = [B, 501, 768] 501个tokens(500 + 1 class token), 直接用class token做class embedding, 因为它汇聚了所有tokens上的信息
        # 注意vit的原理, 所有tokens本是一个序列信息, 选最前的一个class token就行, 即x[B, 0]
        x = self.v.norm(x)
